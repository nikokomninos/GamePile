        ...
        <div className="w-full flex flex-col items-center justify-center">
            ...
            <div>
                <ul className="w-full flex flex-wrap items-center justify-center">
                    ...
                    {results.map((_, index) => (
                        <GameCard id={results[index].id} name={results[index].name} desc={results[index].summary}
                            cover={results[index].cover?.url.replace("t_thumb", "t_cover_big") ?? "/src/assets/fallback.png"}
                            platforms={results[index].platforms}
                            screenshot={results[index].screenshots?.[0]?.url.replace("t_thumb", "t_720p")} to="/gameinfo" />
                    ))}
                </ul>
            </div>
        </div>
