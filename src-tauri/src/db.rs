...
WHERE category=?1",
    ...
    let game_iter = stmt.query_map([category], |row| {
        Ok((
            row.get::<_, i32>(0)?,
            row.get::<_, String>(1)?,
            row.get::<_, String>(2)?,
    ...
    let games: Vec<(i32, String, String, String, String, String)> =
        game_iter.collect::<Result<_, _>>().map_err(|e| e.to_string())?;
...
pub fn remove_game(igdb_id: &i32) -> Result<()> {
    ...
    conn.execute("DELETE FROM backlog WHERE igdb_id=?1",
    params![igdb_id],)?;
